...
bool bNew = false;
cv::Mat fixed_mat;
DetectedObject* fixed_detect_result;
int object_num = 0;
int max_object_num = 20;
...
void init_openvino( std::string architecture, 
    std::string model_xml, std::string labels_path, 
    double probability_threshold, double overlapping_boxes_threshold, std::string device_name ) {
    if ( !labels_path.empty() )
        labels = DetectionModel::loadLabels( labels_path );
    if ( architecture == "centernet" ) {
        model.reset( new ModelCenterNet( model_xml, (float)probability_threshold, labels ) );
    } else if ( architecture == "faceboxes" ) {
        model.reset( new ModelFaceBoxes( model_xml, (float)probability_threshold, false, (float)overlapping_boxes_threshold ) );
    } else if ( architecture == "retinaface" ) {
        model.reset( new ModelRetinaFace( model_xml, (float)probability_threshold, false, (float)overlapping_boxes_threshold ) );
    } else if ( architecture == "retinaface-pytorch" ) {
        model.reset( new ModelRetinaFacePT( model_xml, (float)probability_threshold, false, (float)overlapping_boxes_threshold ) );
    } else if ( architecture == "ssd" ) {
        model.reset( new ModelSSD( model_xml, (float)probability_threshold, false, labels ) );
    } else if ( architecture == "yolo" ) {
        model.reset( new ModelYolo( model_xml, (float)probability_threshold, false, true, (float)overlapping_boxes_threshold, labels, anchors, masks ) );
    } else {
        slog::err << "No model type or invalid model type (-at) provided: " + architecture << slog::endl;
    }

    pipeline = new AsyncPipeline( std::move( model ),
        ConfigFactory::getUserConfig( device_name, FLAGS_l, FLAGS_c, FLAGS_pc, FLAGS_nireq, FLAGS_nstreams, FLAGS_nthreads ),
        core );
}

bool compareByConfidence( const DetectedObject& a, const DetectedObject& b ) {
    return a.confidence > b.confidence; // 按照 score 由大到小排序
}

void infer_callback( std::vector<DetectedObject> detect_result ) {
    //slog::info << "infer_callback" << slog::endl;
    int size = detect_result.size();
    if ( size > max_object_num ) {
        std::sort( detect_result.begin(), detect_result.end(), compareByConfidence );
        object_num = max_object_num;
    } else {
        object_num = size;
    }
    if ( !fixed_detect_result ) {
        fixed_detect_result = (DetectedObject*)malloc( sizeof( DetectedObject ) * max_object_num );
    }
    for ( size_t i = 0; i < object_num; i++ ) {
        memcpy( fixed_detect_result + i, (void*)( &detect_result[i] ), sizeof( DetectedObject ) );
    }
}

void start_object_detect(int limit_num) {
    max_object_num = limit_num;
    keepRunning = true;
    OutputTransform outputTransform = OutputTransform();
    while ( keepRunning ) {
        if ( bNew && pipeline->isReadyToProcess() ) {
            bNew = false;
            ...
            pipeline->submitData( ImageInputData( fixed_mat ),
                std::make_shared<ImageMetaData>( fixed_mat, startTime ) );

        }

        //--- Waiting for free input slot or output data available. Function will return immediately if any of them are available.
        pipeline->waitForData();
        if ( auto result = pipeline->getResult() ) {
            DetectionResult detect_result = result->asRef<DetectionResult>();

            infer_callback( detect_result.objects );
        }
    }

}

void stop_object_detect() {
    keepRunning = false;
}

void infer_mat( cv::Mat curr_frame ) {
    fixed_mat = curr_frame;
    bNew = true;
}
...
int main( int argc, char* argv[] ) {
    try {
        ...
        //------------------------------- Preparing Input ------------------------------------------------------
        slog::info << "Rearinding input" << slog::endl;
        ...
        init_openvino( "yolo",
            "C:\\Users\\willyhuang\\Documents\\Intel\\OpenVINO\\openvino_models\\ir\\public\\yolo-v4-tf\\FP16\\yolo-v4-tf.xml",
            "labels\\coco_80cl.txt",
            0.5,
            0.5,
            "GPU" );        

        std::thread thd_object_detect( start_object_detect, 10 );
        thd_object_detect.detach();
        ...
        while ( keepRunning ) {
            ...
            if ( curr_frame.empty() ) {
                ...
                infer_mat( curr_frame );

                for ( size_t i = 0; i < object_num; i++ ) {
                    auto obj = fixed_detect_result[i];
                    auto color = palette[obj.labelID];
                    cv::rectangle( curr_frame, obj, color, 2 );
                }
            }
